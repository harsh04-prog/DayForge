...
export const HabitStackFlow: React.FC<HabitStackFlowProps> = ({ habits }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {stacks.length === 0 ? (
        <Card className="bg-slate-50/70 dark:bg-[#151724] border-dashed border-slate-300 dark:border-[#2E3348] text-center py-8">
          <Layers className="w-8 h-8 text-slate-400 mx-auto mb-2 opacity-60" />
          <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300">No habit chains created yet</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {stacks.map((stk) => (
            <Card
              key={stk.id}
              className="p-5 bg-white dark:bg-[#151724] border border-slate-200/90 dark:border-[#2E3348] shadow-soft flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  <span className="text-[10px] font-black uppercase tracking-wider text-[#6C5CE7] bg-[#6C5CE7]/10 dark:bg-[#6C5CE7]/20 px-2 py-0.5 rounded-md">
                    Habit Stack Flow
                  </span>
                  <button
                    type="button"
                ...
                <div className="flex items-center gap-3 my-2">
                  <div className="flex-1 p-3 rounded-2xl bg-slate-50 dark:bg-[#181B26] border border-slate-200/80 dark:border-[#2E3348]">
        ...
        <form onSubmit={handleCreateStack} className="space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-[#0F121C] border border-slate-200 dark:border-[#2E3348] rounded-2xl text-sm font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-[#6C5CE7]"
            >
              {habits.map((h) => (
                <option key={h.id} value={h.id} className="bg-white dark:bg-[#181B26] text-slate-900 dark:text-white">
                  {h.name} ({h.category})
                </option>
              ))}
            </select>
          ...
          <div className="flex justify-end gap-2 pt-3 border-t border-slate-100 dark:border-[#2E3348]">
