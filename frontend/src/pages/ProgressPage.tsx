...
export const ProgressPage: React.FC = () => {
    ...
    const loadData = async () => {
      try {
        ...
        const [achRes, txRes] = await Promise.all([
          ...
          api.get<XPTransaction[]>('/progress/transactions'),
        ]);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto text-slate-900">
      ...
      <Card className="bg-gradient-to-br from-white via-indigo-50/40 to-white border border-slate-200/90 shadow-soft p-6 sm:p-8 relative overflow-hidden">
        {/* Subtle Ambient Glow */}
        <div className="absolute -top-20 -right-20 w-60 h-60 rounded-full bg-[#6C5CE7]/10 blur-3xl pointer-events-none" />
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-6 relative z-10">
          ...
          {/* Streak Shields Box */}
          <div className="bg-white border border-slate-200/90 rounded-3xl p-4 text-center sm:text-left min-w-[200px] shadow-xs">
            <div className="flex items-center justify-between gap-2 mb-2">
              <span className="text-xs font-bold text-slate-600">Streak Shields</span>
              <Shield className="w-4 h-4 text-[#6C5CE7] fill-[#6C5CE7]/20" />
            </div>
            <div className="text-2xl font-black text-slate-900 mb-2">
              {availableShields} <span className="text-xs text-slate-400 font-normal">Available</span>
            </div>
            {availableShields > 0 ? (
              <Button
                size="sm"
                ...
                className="w-full bg-slate-50 hover:bg-slate-100 border-[#6C5CE7]/30 text-[#6C5CE7] text-xs font-bold rounded-xl py-1.5"
              >
                Use Streak Shield
              </Button>
            ) : (
        ...
        <div className="mt-8 pt-6 border-t border-slate-200/80 space-y-2 relative z-10">
          <div className="flex items-center justify-between text-xs font-bold">
            <span className="text-slate-600">Progression to Level {level + 1}</span>
          ...
          <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden p-0.5 border border-slate-200/80">
            <div
              className="bg-gradient-to-r from-[#6C5CE7] to-[#8075ea] h-full rounded-full transition-all duration-500 shadow-xs"
              style={{ width: `${pct}%` }}
            />
          </div>
        </div>
      ...
      {activeTab === 'achievements' ? (
          ...
          {achievements.map((ach) => (
            <Card
              ...
              className={`p-5 flex flex-col justify-between transition-all rounded-3xl ${
                ach.unlocked
                  ? 'bg-white border-[#6C5CE7]/30 shadow-soft'
        ...
        /* Transactions List */
        <Card className="bg-white border border-slate-200/90 divide-y divide-slate-100 p-0 overflow-hidden rounded-3xl">
